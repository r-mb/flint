...
padic_float_nmod_ctx_init(gr_ctx_t ctx, ulong p, slong n)
{
    ...
    PADIC_FLOAT_NMOD_CTX_POW(ctx) = (ulong *) flint_malloc(n*sizeof(ulong));

    PADIC_FLOAT_NMOD_CTX_POW(ctx)[0] = p;
    for (i = 1; i < n; i++)
    {
        umul_ppmm(hi, PADIC_FLOAT_NMOD_CTX_POW(ctx)[i],
                      PADIC_FLOAT_NMOD_CTX_POW(ctx)[i - 1], p);
        if (hi != 0)
        {
            padic_float_nmod_ctx_clear(ctx);
            return GR_UNABLE;
        }
    }
    ...
    nmod_init(&PADIC_FLOAT_NMOD_CTX_PN_MOD(ctx), PADIC_FLOAT_NMOD_CTX_POW(ctx)[n-1]);
    for (i = 0; i < n; i++)
    {
        NMOD_RED (PADIC_FLOAT_NMOD_CTX_POW(ctx)[i], PADIC_FLOAT_NMOD_CTX_POW(ctx)[i], PADIC_FLOAT_NMOD_CTX_PN_MOD(ctx));
    }
